    ...
    for page_pk in visible_objects_lists["visible_page_list"]:
        ...
        for widget in page.widget_set.all():
            ...
            if current_row != widget.row:
                ...
                logger.info(pprint.pformat(current_row))
            ...
            #image_html = f'<img src="/static/pyscada/{widget.off_icon}" alt="Widget Image" style="width:100%;">'
            #title_html = f"<h2>{widget.on_ico}</h2>"  # Suponiendo que el widget tiene un campo 'title'
            #image_html = f'<img src="/media/icons/favicon.ico" alt="Widget Image" style="width:100%;">'

           # mc = image_html  # Aquí establecemos solo el título como contenido principal
          #  sbc = None  # Si no quieres contenido en la barra lateral
          #  opts = {}  # Opciones vacías para este caso
            # main content
...
@login_required
def update_widget_position1(request):
    if request.method == 'POST':
...
def widget_detail(request, widget_id):
    ...
    return render(request, 'chart.html', {'widget': widget})
