...
{% if item.active == 1 %}
    ...
    {% if item.display_value_options and item.display_value_options.template and item.display_value_options.template.template_name %}
        <!-- custom template -->
    ...
    {% elif item.type == 0 %} <!-- control element -->
        {% if item.value_class == 'BOOL' or item.value_class == 'BOOLEAN' %}
            <!-- input button -->
            {% include "button.html" with value=item.value key=item.key %}
        {% elif item.control_element_options.dropdown and item.dictionary %}
            <!-- dropdown -->
