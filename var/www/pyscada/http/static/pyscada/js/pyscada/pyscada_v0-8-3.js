...
var loading_labels = {
  ...
  4: "Loading static variables: ",
  5: "Loading chart variables: ",
};
...
function add_fetched_data(key, value) {
  ...
  if (typeof value === "object") {
    ...
    if (value.length > 0) {
      var event = new CustomEvent("pyscadaVariableDataChange-" + key, {
        bubbles: false,
        cancelable: true,
        composed: false,
      });
            ...
            if (typeof stop_id === "number") {
              ...
              if (typeof start_id === "number") {
                ...
                console.log(
                  "PyScada HMI : var",
                  key,
                  ": dropped data, start_id not found.",
                  value,
                  DATA[key][0][0]
                );
              }
              ...
              console.log(
                "PyScada HMI : var",
                key,
                ": dropped data, stop_id not found.",
                value,
                DATA[key][DATA[key].length - 1][0]
              );
            }
...
function dictionary(id, val, type) {
  ...
  if (typeof dict != "undefined") {
    ...
    for (item in l) {
      if (
        l[item] == dict &&
        (get_config_from_hidden_config("dictionaryitem", "id", item, "value") ==
          val ||
          get_config_from_hidden_config(
            "dictionaryitem",
            "id",
            item,
            "value"
        ...
        // last check : int stored as a float
        val = get_config_from_hidden_config(
          "dictionaryitem",
          "id",
          item,
          "label"
...
function update_data_colors(id, val) {
  ...
  function rgbToHex(rgb) {
    if (typeof rgb != "object" || rgb.constructor != Array || rgb.length != 3) {
      console.log(
        "PyScada HMI : cannot update data colors, rgb to hex error :",
        typeof rgb,
        rgb.constructor,
        rgb.length,
        rgb
  ...
  var display_value_option_id = get_config_from_hidden_config(
    "controlitem",
    "id",
    id,
  ...
  // variable colors
  var color_only = Number(
    get_config_from_hidden_config(
      "displayvalueoption",
      "id",
      display_value_option_id,
      "color-only"
    )
  );
  var gradient = get_config_from_hidden_config(
    "displayvalueoption",
    "id",
    display_value_option_id,
    "gradient"
  ...
  var gradient_higher_level = Number(
    get_config_from_hidden_config(
      "displayvalueoption",
      "id",
      display_value_option_id,
      "gradient-higher-level"
    )
  );
  var color_init = get_config_from_hidden_config(
    "displayvalueoption",
    "id",
    display_value_option_id,
    "color"
  ...
  var colors = [];
  var color_options = get_config_from_hidden_configs(
  ...
  for (dvco in color_options) {
    if (display_value_option_id == color_options[dvco]) {
      ...
      var color = get_config_from_hidden_config(
        "displayvaluecoloroption",
        "id",
        dvco,
        "color"
  ...
  var v_id = null;
  if (
    get_config_from_hidden_config("controlitem", "id", id, "variable") != "None"
  ) {
    type = "variable";
    ...
    v_id = get_config_from_hidden_config(
      "controlitem",
      "id",
      id,
      "variable-property"
  ...
  if (
    get_config_from_hidden_config(type, "id", v_id, "value-class") == "BOOLEAN"
  ) {
    if (val == false) {
      val = 0;
    } else if (val == true) {
  ...
  if (typeof color_init == "undefined") {
    return;
  }
  ...
  switch (gradient) {
    ...
    case 1:
      if (colors.length == 0) {
        return;
      } // Need one display value color option
      if (val <= colors[0]["level"]) {
        ...
        var fade =
          (val - colors[0]["level"]) /
          (gradient_higher_level - colors[0]["level"]);
        var color_1_new = new Color(
          Number(get_config_from_hidden_config("color", "id", color_init, "r")),
          Number(get_config_from_hidden_config("color", "id", color_init, "g")),
          Number(get_config_from_hidden_config("color", "id", color_init, "b"))
        );
        var color_2_new = new Color(
          Number(
            get_config_from_hidden_config(
              "color",
              "id",
...
function transform_data(control_item_id, val, key) {
  ...
  var transform_data_id = get_config_from_hidden_config(
    "displayvalueoption",
    "id",
    display_value_option_id,
    "transform-data"
  );
  var transform_data_function_name = get_config_from_hidden_config(
    "transformdata",
    "id",
    transform_data_id,
    "js-function-name"
  ...
  if (
    ...
    if (typeof window[transform_data_function_name] === "function") {
      ...
      console.log(
        "PyScada HMI : " + transform_data_function_name + " function not found."
      );
    }
  }
  return val;
...
function update_data_values(key, data) {
  ...
  if (key.split("-")[0] == "var") {
    ...
    var type = "variable-property";
  }

  // get the unit
  var unit_id = get_config_from_hidden_config(
    type,
    "id",
    key.split("-")[1],
    "unit"
  ...
  if (type == "variable") {
    ...
    var var_id = get_config_from_hidden_config(
      type,
      "id",
      key.split("-")[1],
      "variable"
  ...
  var device_id = get_config_from_hidden_config(
    "variable",
    "id",
    var_id,
    "device"
  );
  var device_polling_interval = get_config_from_hidden_config(
    "device",
    "id",
    device_id,
    "polling-interval"
  ...
  if (typeof data === "object" && data !== null) {
    ...
    document
      .querySelectorAll(
        ".control-item.type-numeric." + key + ", button.write-task-btn." + key
      )
      .forEach(function (e) {
        ...
        var display_value_option_id = get_config_from_hidden_config(
          "controlitem",
          "id",
          control_item_id.split("-")[1],
          "display-value-options"
        );
        var from_timestamp_offset = get_config_from_hidden_config(
          "displayvalueoption",
          "id",
          display_value_option_id,
          "from-timestamp-offset"
        );
        var var_id = get_config_from_hidden_config(
          "controlitem",
          "id",
        ...
        var var_readable = get_config_from_hidden_config(
          "variable",
          "id",
          var_id,
          "readable"
        ...
        var glyphicon_exclamation = e.parentElement.querySelector(
          ".glyphicon-exclamation-sign"
        );
        var status_icon = e.parentElement.querySelector(".status-icon");
        ...
        if (data.length) {
          ...
          if (isNaN(from_timestamp_offset)) {
            ...
            var data_temp = sliceDATAusingTimestamps(
              data,
              (display_from =
                DATA_DISPLAY_FROM_TIMESTAMP - from_timestamp_offset)
            );
          }
          if (data_temp.length) {
        ...
        function toggleState(id) {
          var icon = document.getElementById("status-icon-" + id);
          var isOn = icon.src.includes("{{ widget.on_icon.url }}");

          // Cambiar el estado usando las imágenes del modelo
          icon.src = isOn
            ? "{{ widget.off_icon.url }}"
            : "{{ widget.on_icon.url }}";

          // Opcionalmente, enviar una solicitud AJAX para actualizar el estado en el servidor
          fetch("/update-state", {
            method: "POST",
            headers: {
        ...
        if (time != null) {
          ...
          var t_next_update_string =
            t_next_update < 1000 ? "< 1 sec" : msToTime(t_next_update);
          var tooltip_text =
            "Current value: " +
            val +
            " " +
          ...
          if (var_readable === "True") {
            ...
            if (
              time <
              SERVER_TIME -
                10 * Math.max(1000 * device_polling_interval, REFRESH_RATE)
            ) {
              glyphicon_alert !== null
                ? glyphicon_alert.classList.remove("hidden")
                : null;
              glyphicon_exclamation !== null
                ? glyphicon_exclamation.classList.add("hidden")
                : null;
            } else if (
              time <
              SERVER_TIME -
                3 * Math.max(1000 * device_polling_interval, REFRESH_RATE)
            ) {
              glyphicon_alert !== null
                ? glyphicon_alert.classList.add("hidden")
                : null;
              glyphicon_exclamation !== null
                ? glyphicon_exclamation.classList.remove("hidden")
                : null;
            } else {
              glyphicon_alert !== null
                ? glyphicon_alert.classList.add("hidden")
        ...
        var refresh_requested_timestamp = parseFloat(
          get_config_from_hidden_config(
            type.replace("-", ""),
            "id",
            key.split("-")[1],
            "refresh-requested-timestamp"
          )
        );
        if (
          refresh_requested_timestamp != null &&
          time != null &&
          time <= refresh_requested_timestamp
        ) {
          return;
        }
        ...
        if (typeof val === "number" || typeof val === "boolean") {
          ...
          var color_only = get_config_from_hidden_config(
            "displayvalueoption",
            "id",
            display_value_option_id,
            "color-only"
          );
          var timestamp_conversion_value = get_config_from_hidden_config(
            "displayvalueoption",
            "id",
            display_value_option_id,
            "timestamp-conversion"
          );
          var ci_label = get_config_from_hidden_config(
            "controlitem",
            "id",
            control_item_id.split("-")[1],
            "label"
          );
          var temp_val = transform_data(
            control_item_id.split("-")[1],
            val,
            key
          );
          ...
          if (typeof temp_val === "number") {
            ...
            if ((r_val === 0) | (r_val == false)) {
              $("button.btn-success.write-task-btn." + key).addClass(
                "update-able"
              );
              $("button.update-able.write-task-btn." + key).addClass(
                "btn-default"
              );
              $("button.update-able.write-task-btn." + key).removeClass(
                "btn-success"
              );
              ...
              $("button.btn-default.write-task-btn." + key).addClass(
                "update-able"
              );
              $("button.update-able.write-task-btn." + key).removeClass(
                "btn-default"
          ...
          if (typeof color_only == "undefined" || color_only != "True") {
            if (typeof val === "number") {
              if (
                timestamp_conversion_value != null &&
                timestamp_conversion_value != 0 &&
                typeof timestamp_conversion_value != "undefined"
              ) {
              ...
              typeof temp_val === "boolean" &&
              e.querySelector(".boolean-value") != null
            ) {
              // Set the text value
              e.querySelector(".boolean-value").innerHTML =
                ci_label +
                " : " +
                dictionary(var_id, temp_val, type.replace("-", "")) +
                " " +
                unit;
            }
          }
          if (display_value_option_id != "None") {
            ...
            if (e.classList.contains("process-flow-diagram-item")) {
              ...
              e.style.backgroundColor = update_data_colors(
                control_item_id,
                temp_val
              );
            }
            ...
            var event = new CustomEvent(
              "changePyScadaControlItemColor_" + control_item_id.split("-")[1],
              { detail: update_data_colors(control_item_id, val) }
            );
    ...
    if (DATA_DISPLAY_FROM_TIMESTAMP > 0 && time < DATA_DISPLAY_FROM_TIMESTAMP) {
      ...
      DATA_DISPLAY_TO_TIMESTAMP > 0 &&
      time > DATA_DISPLAY_TO_TIMESTAMP
    ) {
...
*/
function get_config_from_hidden_configs(
  type,
  filter_data = "id",
  get_data = "id"
) {
  ...
  if (
    typeof type !== "string" ||
    typeof filter_data !== "string" ||
    typeof get_data !== "string" ||
    filter_data === "" ||
    get_data === "" ||
    type === ""
  ) {
...
*/
function set_config_from_hidden_config(
  type,
  filter_data,
  val,
  get_data,
  ...
  document
    .querySelectorAll(
      "." + type + "-config2[data-" + filter_data + "='" + val + "']"
    )
    .forEach(function (e) {
...
function add_key_to_chart_vars(
  key,
  var_count,
  vars,
  device_pulling_interval_sum,
  timestamp,
  var_count_poll
) {
  if (
    key in CHART_VARIABLE_KEYS &&
    CHART_VARIABLE_KEYS[key] <= DATA_INIT_STATUS
  ) {
    CHART_VARIABLE_KEYS[key]++;
    var_count++;
    INIT_CHART_VARIABLES_COUNT++;
    vars.push(key);
    dpi = get_config_from_hidden_config(
      "device",
      "id",
      get_config_from_hidden_config("variable", "id", key, "device"),
      "polling-interval"
    );
    if (!isNaN(dpi)) {
      device_pulling_interval_sum += parseFloat(dpi);
      var_count_poll++;
    } else {
      console.log("PyScada HMI : ConfigV2 not found for var", key);
    }
...
function data_handler() {
  if (
    AUTO_UPDATE_ACTIVE ||
    !INIT_STATUS_VARIABLES_DONE ||
    !INIT_CHART_VARIABLES_DONE
  ) {
    if (DATA_TO_TIMESTAMP == 0 && FETCH_DATA_PENDING <= 0) {
      ...
      // fetch historic data
        ...
        if (!INIT_STATUS_VARIABLES_DONE) {
          ...
          if (var_count > 0) {
            ...
            set_loading_state(
              4,
              (loading_states[4] || 0) +
                (100 * var_count) / STATUS_VARIABLE_KEYS.count()
            );
          } else {
          ...
          var page = document.querySelector(
            '#content .sub-page:not([style*="display:none"]):not([style*="display: none"])'
          );
          var visible_hidden_config =
            page != null
              ? page.querySelectorAll(".hidden.variable-config")
              : [];
          var visible_vars = [];
          ...
          if (var_count > 0) {
            ...
            if (var_count_poll > 0) {
              duration_for_quantity =
                (point_quantity_to_fetch_by_var * device_pulling_interval_sum) /
                var_count_poll;
        ...
        set_loading_state(
          ...
          (loading_states[5] || 0) +
            100 *
              (vars.length / CHART_VARIABLE_KEYS.count()) *
              ((timestamp - t) / (tmax - t_start))
        );
  ...
  if (!INIT_STATUS_VARIABLES_DONE || !INIT_CHART_VARIABLES_DONE) {
    ...
    if (
      STATUS_VARIABLE_KEYS.count() + CHART_VARIABLE_KEYS.count() == 0 &&
      LOADING_PAGE_DONE == 0
    ) {
      LOADING_PAGE_DONE = 1;
      show_page();
    ...
    PYSCADA_TIMEOUTS["data_handler"] = setTimeout(function () {
      data_handler();
    }, 100);
  } else {
    if (LOADING_PAGE_DONE == 0) {
      LOADING_PAGE_DONE = 1;
      show_page();
      hide_loading_state();
      loading_states = {};
...
function data_handler_ajax(
  ...
  request_data = {
    ...
    init: init,
    variable_properties: variable_property_keys,
  };
  ...
  var timer = setTimeout(() => {
    add_notification("Timeout while fetching data", 3, 0.9 * 5 * REFRESH_RATE);
    controller.abort();
  }, timeout);
  const signal = controller.signal;
  FETCH_CONTROLLERS["data_handler_ajax"] = controller;
  fetch(ROOT_URL + "json/cache_data/", {
    method: "POST",
    ...
    .then((response) => {
      ...
      if (response.redirected) {
        ...
        if (response.url.includes(next)) {
          url_redirect =
            url_redirect.split(next)[0] + next + window.location.href;
        }
        if (AUTO_UPDATE_ACTIVE) {
          auto_update_click();
        }
        add_notification(
          "Authentication issue, please log-in <a href='" +
            url_redirect +
            "'>here</a>",
          3,
          0,
    ...
    .then((json) => {
      data_handler_done(json);
    })
    .catch((error) => {
      if (timer === null) {
        add_notification("data_handler_ajax : " + error, 3, 0.9 * REFRESH_RATE);
      }
...
function data_handler_done(fetched_data) {
  if (typeof fetched_data === "undefined") {
    console.log("PyScada HMI : fetched data undefined");
    return;
  }
  ...
  if (typeof fetched_data["server_time"] === "number") {
    ...
    $(".server_time").html(
      date.toLocaleString() + " (" + createOffset(date) + " GMT)"
    );
  } else {
    SERVER_TIME = 0;
  }
  ...
  if (typeof fetched_data["variable_properties"] === "object") {
    ...
    VARIABLE_PROPERTIES_LAST_MODIFIED =
      fetched_data["variable_properties_last_modified"];
  ...
  if (DATA_TO_TIMESTAMP == 0) {
    ...
    DEFAULT_TIME_DELTA =
      document.querySelector("body").getAttribute("data-view-time-delta") ==
      null
        ? 7200
        : parseFloat(
            document.querySelector("body").getAttribute("data-view-time-delta")
...
function data_handler_fail(response) {
  ...
  if (JSON_ERROR_COUNT % 5 == 0) {
    add_notification(
      "Fetching data failed (" + response.status + ")",
      3,
      0.95 * 5 * REFRESH_RATE
    );
  }
...
function increase_error_count() {
  ...
  if (JSON_ERROR_COUNT > 15) {
    ...
    auto_update_click();
    add_notification(
      "Fetching data failed limit reached, auto update deactivated.<br>Check your connectivity and active auto update in the top right corner.",
      2,
      0
    );
...
function get_period_fields(key) {
  calculatedvariableselectorID = Number(
    get_config_from_hidden_config(
  ...
  if (
    typeof calculatedvariableselectorID == "undefined" ||
    isNaN(calculatedvariableselectorID)
  ) {
    return key;
  }
  periodFields = get_config_from_hidden_config(
    "calculatedvariableselector",
    "id",
    calculatedvariableselectorID,
    "period-fields"
  ).split(",");
...
function filter_aggregation_type_for_period_list(period_list) {
  ...
  for (p in period_list) {
    i = get_config_from_hidden_config(
      "periodicfield",
      "id",
      period_list[p],
      "type"
...
function get_one_field_by_period_field(validPeriodFields) {
  ...
  for (validPeriodField in validPeriodFields) {
    period = get_config_from_hidden_config(
      "periodicfield",
      "id",
      validPeriodFields[validPeriodField],
      "period"
    );
    if (periodCalcVar[period] == null) {
      periodCalcVar[period] = validPeriodFields[validPeriodField];
    } else {
      newPeriodFactor = get_config_from_hidden_config(
        "periodicfield",
        "id",
        validPeriodFields[validPeriodField],
        "period-factor"
      );
      newStartFrom = get_config_from_hidden_config(
        "periodicfield",
        "id",
        validPeriodFields[validPeriodField],
        "period-factor"
      );
      currentPeriodFactor = get_config_from_hidden_config(
        "periodicfield",
        "id",
        periodCalcVar[period],
        "period-factor"
      );
      currentStartFrom = get_config_from_hidden_config(
        "periodicfield",
        "id",
        periodCalcVar[period],
        "period-factor"
      ...
      if (currentPeriodFactor > newPeriodFactor) {
        ...
        (currentPeriodFactor =
          newPeriodFactor && currentStartFrom > newStartFrom)
      ) {
        periodCalcVar[period] = validPeriodFields[validPeriodField];
      }
    }
...
function get_variable_keys_for_period_calculated_variables(periodCalcVar) {
  ...
  for (p in periodCalcVar) {
    if (periodCalcVar[p] !== null) {
      periods = get_config_from_hidden_configs(
        "calculatedvariable",
        "id",
        "period"
      );
      for (idP in periods) {
        if (
          periodCalcVar[p] == periods[idP] &&
          get_config_from_hidden_config(
            "calculatedvariable",
            "id",
            idP,
            "variable-calculated-fields"
          ) == calculatedvariableselectorID
        ) {
          keyCalcVar[p] = get_config_from_hidden_config(
            "calculatedvariable",
            "id",
            idP,
            "store-variable"
...
function get_one_variable_key_of_calculated_variable_for_duration(
  start,
  stop,
  min_aggregate,
  keyCalcVar
  ...
  if (
    keyCalcVar[6] !== null &&
    stop - start > 60 * 60 * 24 * 365 * min_aggregate
  ) {
    return keyCalcVar[6];
  } else if (
    keyCalcVar[5] !== null &&
    stop - start > 60 * 60 * 24 * 31 * min_aggregate
  ) {
    return keyCalcVar[5];
  } else if (
    keyCalcVar[4] !== null &&
    stop - start > 60 * 60 * 24 * 7 * min_aggregate
  ) {
    return keyCalcVar[4];
  } else if (
    keyCalcVar[3] !== null &&
    stop - start > 60 * 60 * 24 * min_aggregate
  ) {
    return keyCalcVar[3];
  } else if (keyCalcVar[2] !== null && stop - start > 60 * 60 * min_aggregate) {
    return keyCalcVar[2];
  } else if (keyCalcVar[1] !== null && stop - start > 60 * min_aggregate) {
    return keyCalcVar[1];
  } else if (keyCalcVar[0] !== null && stop - start > min_aggregate) {
    return keyCalcVar[0];
  }
  return null;
}
...
function get_aggregated_data(key, start, stop, type = 6, min_aggregate = 3) {
  if (
    ...
    !Number.isInteger(min_aggregate)
  ) {
    console.log(
      "PyScada HMI : get_aggregated_data : params types are not int, number, number, int : ",
      key,
      start,
      stop,
      type
  ...
  if (min_aggregate <= 0) {
    console.log(
      "PyScada HMI : min_aggregate should be > 0, it is ",
      min_aggregate
    );
    return key;
  }
  ...
  if (start < 0) {
    start = DATA_FROM_TIMESTAMP;
  }
  if (stop < 0) {
    stop = DATA_TO_TIMESTAMP;
  }
  start = start / 1000;
  stop = stop / 1000;
  if (start >= stop) {
    console.log("PyScada HMI : start is not < stop :", start, stop);
    return key;
  }
  if (type < 0 || type > 14) {
    console.log("PyScada HMI : type is not 0<= and >=14 :", type);
  }
  ...
  new_key = get_one_variable_key_of_calculated_variable_for_duration(
    start,
    stop,
    min_aggregate,
    keyCalcVar
...
function PyScadaPlot(id, xaxisVarId, xaxisLinLog) {
  var options = {
      ...
      xaxis: {
        ...
        ticks:
          xaxisVarId == null
            ? $("#chart-container-" + id).data("xaxisTicks")
            : null,
  ...
  plot.getFlotObject = function () {
    return flotPlot;
  };
  plot.getKeys = function () {
    return keys;
  };
  plot.getVariableNames = function () {
    return variable_names;
  };

  plot.getInitStatus = function () {
    if (InitDone) {
      return InitRetry;
    } else {
      return false;
  ...
  plot.getChartContainerId = function () {
    return chart_container_id;
  };
  ...
  tf = function (value, axis) {
    return (
      ...
      (typeof options.yaxes[axis.n - 1].unit != "undefined" &&
      options.yaxes[axis.n - 1].unit != null
        ? options.yaxes[axis.n - 1].unit
        : "")
    );
  };
  ...
  $.each($(legend_table_id + " .variable-config"), function (key, val) {
    ...
    variables[variable_key] = {
      color: val_inst.data("color"),
      yaxis: raxis_id,
      axis_id: axis_id,
    };
    keys.push(variable_key);
    variable_names.push(variable_name);
    variables[variable_key].label = $(
      ".legendLabel[data-key=" + variable_key + "] .legendLabel-text"
    )[0].textContent.replace(/\s/g, "");
    variables[variable_key].unit = $(
      ".legendUnit[data-key=" + variable_key + "]"
    )[0].textContent.replace(/\s/g, "");
    ...
    options.yaxes[raxis_id].axisLabel =
      options.yaxes[raxis_id].label.replace(/\s/g, "") +
      (typeof options.yaxes[raxis_id].unit != "undefined" &&
      options.yaxes[raxis_id].unit != "" &&
      options.yaxes[raxis_id].unit != null
        ? " (" + options.yaxes[raxis_id].unit + ")"
        : "");
  });
  ...
  function updateLegend() {
    var pos = flotPlot.c2p({
      left: flotPlot.crosshair_x,
      top: flotPlot.crosshair_y,
    });
  ...
  function prepare() {
    ...
    flotPlot = $.plot(
      $(chart_container_id + " .chart-placeholder"),
      series,
      options
    );
    ...
    $(chart_container_id + " .chart-placeholder")
      .on("plothover", function (event, pos, item) {
        ...
        for (axis in pos) {
          if (!$("#" + axis + "-tooltip").length) {
            $("<div id='" + axis + "-tooltip' class='axes-tooltips'></div>")
              .css({
                position: "absolute",
                display: "none",
        ...
        if (
          item &&
          typeof item.datapoint != "undefined" &&
          item.datapoint.length > 1
        ) {
          opts = item.series.xaxis.options;
          if (opts.mode == "time") {
            dG = $.plot.dateGenerator(
              Number(item.datapoint[0].toFixed(0)),
              opts
            );
            dF = $.plot.formatDate(
              dG,
          ...
          y_label =
            typeof item.series.label !== "undefined" ? item.series.label : "T";
          y_unit =
            typeof item.series.unit !== "undefined" ? item.series.unit : "";
          $("#tooltip")
            .html(y_label + " (" + x + ") = " + y + " " + y_unit)
        ...
        flotPlot.crosshair_x = Math.max(
          0,
          Math.min(pos.pageX - offset.left - plotOffset.left, flotPlot.width())
        );
        flotPlot.crosshair_y = Math.max(
          0,
          Math.min(pos.pageY - offset.top - plotOffset.top, flotPlot.height())
        );
      ...
      .on("mousedown", function (e) {
        ...
        pos.x = Math.max(
          0,
          Math.min(e.pageX - offset.left - plotOffset.left, flotPlot.width())
        );
        pos.y = Math.max(
          0,
          Math.min(e.pageY - offset.top - plotOffset.top, flotPlot.height())
        );
      ...
      .on("plotselected", function (event, ranges) {
        ...
        // activate zoom x
        if (
          $(chart_container_id + " .activate_zoom_x").is(":checked") &&
          ranges.xaxis != null
        ) {
          if (xaxisVarId == null) {
            DATA_DISPLAY_TO_TIMESTAMP =
              DATA_TO_TIMESTAMP == ranges.xaxis.to
                ? DATA_DISPLAY_TO_TIMESTAMP
                : ranges.xaxis.to;
            DATA_DISPLAY_FROM_TIMESTAMP =
              DATA_FROM_TIMESTAMP == ranges.xaxis.from
                ? DATA_DISPLAY_FROM_TIMESTAMP
                : ranges.xaxis.from;
            if (
              DATA_DISPLAY_TO_TIMESTAMP < 0 &&
              ...
              DATA_DISPLAY_WINDOW = DATA_TO_TIMESTAMP - DATA_FROM_TIMESTAMP;
            } else if (DATA_DISPLAY_TO_TIMESTAMP < 0) {
              DATA_DISPLAY_WINDOW =
                DATA_TO_TIMESTAMP - DATA_DISPLAY_FROM_TIMESTAMP;
            } else if (DATA_DISPLAY_FROM_TIMESTAMP < 0) {
              DATA_DISPLAY_WINDOW =
                DATA_DISPLAY_TO_TIMESTAMP - DATA_FROM_TIMESTAMP;
            } else {
              DATA_DISPLAY_WINDOW =
                DATA_DISPLAY_TO_TIMESTAMP - DATA_DISPLAY_FROM_TIMESTAMP;
            }
            set_x_axes();
          } else {
    ...
    $(chart_container_id + " .btn.btn-default.chart-save-csv").click(
      function () {
    ...
    $(chart_container_id + " .btn.btn-default.chart-save-picture").click(
      function () {
        var originalCanvas1 = $(chart_container_id + " .flot-base")[0];
        var originalCanvas2 = $(chart_container_id + " .flot-overlay")[0];
        var originalCanvas3 = $(chart_container_id + " .flot-svg")[0]
          .children[0];
        ...
        setTimeout(function () {
          download(
            $("#myCanvas")[0].toDataURL("image/png"),
            "image.png",
            "image/png"
          );
        }, 500);
    ...
    $(chart_container_id + " .btn.btn-default.chart-ResetSelection").click(
      function () {
        e = jQuery.Event("click");
        jQuery(
          chart_container_id + " .btn.btn-default.chart-ZoomYToFit"
        ).trigger(e);
        jQuery(
          chart_container_id + " .btn.btn-default.chart-ZoomXToFit"
        ).trigger(e);
      }
    );
    // chart zoom y to fit button
    $(chart_container_id + " .btn.btn-default.chart-ZoomYToFit").click(
      function () {
        pOpt = flotPlot.getOptions();
        for (y in pOpt.yaxes) {
    ...
    $(chart_container_id + " .btn.btn-default.chart-ZoomXToFit").click(
      function () {
        if (xaxisVarId == null) {
  ...
  function update(force) {
    ...
    if ($(chart_container_id).is(":visible") || force) {
      ...
      for (var key in keys) {
        ...
        aggregatedtType = document.querySelector(
          legend_table_id +
            ' .aggregation-type-option[data-id="' +
            original_key +
            '"]'
        );
        if (
          aggregatedtType !== null &&
          !isNaN(parseInt(aggregatedtType.value))
        ) {
          key = Number(
            get_aggregated_data(
              keys[key],
              DATA_DISPLAY_FROM_TIMESTAMP,
              DATA_DISPLAY_TO_TIMESTAMP,
              parseInt(aggregatedtType.value)
            )
          );
        } else {
          key = original_key;
        ...
        if (key == original_key) {
          variables[key].label = $(
            ".legendLabel[data-key=" + original_key + "] .legendLabel-text"
          )[0].textContent.replace(/\s/g, "");
        } else {
          variables[key].label =
            $(
            ...
            " " +
            $(
              ".legendLabel[data-key=" + original_key + "] span"
            )[1].textContent.replace(/\s/g, "");
        }
        ...
        if (
          $(legend_checkbox_id + original_key).is(":checked") &&
          ...
          if (
            ...
            start_id = find_index_sub_gte(
              DATA[key],
              DATA_DISPLAY_FROM_TIMESTAMP,
              0
            );
            stop_id = find_index_sub_lte(
              DATA[key],
              DATA_DISPLAY_TO_TIMESTAMP,
              0
            );
          ...
          if (xkey == null) {
            for (serie in old_series) {
              if (
                new_data_bool === false &&
                chart_data.length > 0 &&
                key === old_series[serie]["key"] &&
                chart_data.length !== old_series[serie]["data"].length &&
                (old_series[serie]["data"].length == 0 ||
                  chart_data[0][0] !== old_series[serie]["data"][0][0] ||
                  chart_data[0][1] !== old_series[serie]["data"][0][1] ||
                  (chart_data[chart_data.length - 1][0] !==
                    old_series[serie]["data"][
                      old_series[serie]["data"].length - 1
                    ][0] &&
            ...
            series.push({
              ...
              yaxis: variables[key].yaxis + 1,
              label: variables[key].label,
              unit: variables[key].unit,
              key: key,
              points: { show: axes[variables[key].yaxis].points },
              stack: axes[variables[key].yaxis].stack,
              bars: { show: axes[variables[key].yaxis].bars },
              lines: {
                show: axes[variables[key].yaxis].lines,
                steps: axes[variables[key].yaxis].steps,
                fill: axes[variables[key].yaxis].fill,
              },
            });
            ...
            if (
              ...
              start_xid = find_index_sub_gte(
                DATA[xkey],
                DATA_DISPLAY_FROM_TIMESTAMP,
                0
              );
              stop_xid = find_index_sub_lte(
                DATA[xkey],
                DATA_DISPLAY_TO_TIMESTAMP,
                0
              );
                ...
                if (chart_x_data.length > 1) {
                  while (ix < chart_x_data.length && xf == 0) {
                    if (chart_x_data[ix][0] >= chart_data[iy][0]) {
                      ...
                      chart_data_min = Math.min(
                        chart_data_min,
                        chart_data[iy][1]
                      );
                      chart_data_max = Math.max(
                        chart_data_max,
                        chart_data[iy][1]
                      );
                  ...
                  if (xf == 0) {
                    fx = linearInterpolation(
                      chart_data[iy][0],
                      chart_x_data[chart_x_data.length - 2][0],
                      chart_x_data[chart_x_data.length - 2][1],
                      chart_x_data[chart_x_data.length - 1][0],
                      chart_x_data[chart_x_data.length - 1][1]
                    );
                    new_data.push([fx, chart_data[iy][1]]);
                    chart_data_min = Math.min(
                      chart_data_min,
            ...
            if (new_data.length > 0) {
              ...
              for (serie in old_series) {
                if (
                  ...
                  new_data.length > 0 &&
                  key === old_series[serie]["key"] &&
                  new_data.length !== old_series[serie]["data"].length &&
                  (old_series[serie]["data"].length == 0 ||
                    new_data[0][0] !== old_series[serie]["data"][0][0] ||
                    new_data[0][1] !== old_series[serie]["data"][0][1] ||
                    (new_data[new_data.length - 1][0] !==
                      old_series[serie]["data"][
                        old_series[serie]["data"].length - 1
                      ][0] &&
                    ...
                    chart_x_data[0][0] !== old_series[serie]["xdata"][0][0] ||
                    chart_x_data[0][1] !== old_series[serie]["xdata"][0][1] ||
                    (chart_x_data[chart_x_data.length - 1][0] !==
                      old_series[serie]["xdata"][
                        old_series[serie]["xdata"].length - 1
                      ][0] &&
                      chart_x_data[chart_x_data.length - 1][1] !==
                        old_series[serie]["xdata"][
                          old_series[serie]["xdata"].length - 1
              ...
              series.push({
                ...
                label: variables[key].label,
                unit: variables[key].unit,
                chart_data_min: chart_data_min,
                chart_data_max: chart_data_max,
      ...
      if (
        ...
        old_series.length == 0 ||
        series.length == 0 ||
        old_series.length != series.length ||
        force
      ) {
        //update y window
        pOpt = flotPlot.getOptions();
        ...
        if (xaxisVarId !== null && jk != 1) {
          for (k = 1; k <= jk; k++) {
            ...
            if (typeof S !== "undefined") {
              $(
                chart_container_id + " .axisLabels.y" + S["yaxis"] + "Label"
              ).css("fill", S["color"]);
              $(
                chart_container_id + " .flot-y" + S["yaxis"] + "-axis text"
              ).css("fill", S["color"]);
            }
          }
        }
...
function labelFormatter(label, series) {
  return (
    "<div style='font-size:8pt; text-align:center; padding:2px; color:" +
    series.color +
    ";'>" +
    label +
    "<br/>" +
...
function Pie(id, radius, innerRadius) {
  ...
  $.each($(legend_table_id + " .variable-config"), function (key, val) {
    ...
    $.each($(legend_table_id + " .legendSeries"), function (kkey, val) {
      ...
      if (variable_key == val_inst.find(".variable-config").data("key")) {
        variables[variable_key].label = val_inst
          .find(".legendLabel")
          .text()
          .replace(/\s/g, "");
        variables[variable_key].unit = val_inst
          .find(".legendUnit")
          .text()
          .replace(/\s/g, "");
      }
  ...
  function prepare() {
    ...
    $(chart_container_id + " .chart-placeholder").on(
      "plothover",
  ...
  function update(force) {
    ...
    if (prepared && ($(chart_container_id).is(":visible") || force)) {
      ...
      for (var key in keys) {
        ...
        if (
          $(legend_checkbox_id + key).is(":checked") &&
          typeof d === "object"
        ) {
          series.push({
            data: d,
      ...
      if (series.length > 0 || force) {
        if (typeof flotPlot !== "undefined") {
          ...
          // remove old errors
          document
            .querySelectorAll(chart_container_id + " .chart-placeholder .error")
            .forEach((e) => e.remove());
          flotPlot.draw();
        } else {
          flotPlot = $.plot(
...
function set_chart_selection_mode() {
  ...
  $.each($(".xy-chart-container"), function (key, val) {
    ...
    if (
      $("#xy-chart-container-" + id + " .activate_zoom_x").is(":checked") &&
      $("#xy-chart-container-" + id + " .activate_zoom_y").is(":checked")
    ) {
      mode = "xy";
    } else if (
      $("#xy-chart-container-" + id + " .activate_zoom_y").is(":checked")
    ...
    $.each(PyScadaPlots, function (plot_id) {
      if (
        typeof PyScadaPlots[plot_id].getFlotObject() !== "undefined" &&
        PyScadaPlots[plot_id].getId() === id
      ) {
        PyScadaPlots[plot_id].getFlotObject().getOptions().selection.mode =
          mode;
      }
    });
  });
  $.each($(".chart-container"), function (key, val) {
    ...
    // active the zoom mode according to the checked checkbox
    if (
      $("#chart-container-" + id + " .activate_zoom_x").is(":checked") &&
      $("#chart-container-" + id + " .activate_zoom_y").is(":checked")
    ) {
      mode = "xy";
    } else if (
      $("#chart-container-" + id + " .activate_zoom_y").is(":checked")
...
function update_timeline() {
  ...
  if (DATA_DISPLAY_FROM_TIMESTAMP < 0) {
    var min_from = Math.min(
      min_full,
      (DATA_TO_TIMESTAMP - DATA_FROM_TIMESTAMP) / 60 / 1000
    );
    $("#timeline-time-from-label").html("");
  } else {
    var min_from = Math.min(
      min_full,
      (DATA_TO_TIMESTAMP - DATA_DISPLAY_FROM_TIMESTAMP) / 60 / 1000
    );
  ...
  if (DATA_DISPLAY_FROM_TIMESTAMP < 0 && DATA_DISPLAY_TO_TIMESTAMP < 0) {
    ...
    $("#timeline").css(
      ...
      Math.min(
        100,
        (DATA_DISPLAY_WINDOW / 60 / 1000 / min_full) * 100
      ).toString() + "%"
    );
    $("#timeline").css(
      "left",
      Math.max(0, Math.min(100 - (min_from / min_full) * 100, 100)).toString() +
        "%"
    );
  }
...
function set_datetimepicker() {
  if ($(".show_daterangepicker").length) {
    $("#daterange").daterangepicker(
      ...
      function (start, end, label) {
        ...
        if (
          label.indexOf("Last") !== -1 ||
          label.indexOf("Today") !== -1 ||
          label.indexOf("This Month") !== -1 ||
          label.indexOf("This Year") !== -1
        ) {
        ...
        var event = new CustomEvent("pyscadaDateTimeChange", {
          detail: { picker: this },
          bubbles: false,
          cancelable: true,
          composed: false,
        ...
        document
          .querySelectorAll(".pyscadaDateTimeChange")
    ...
    $("#daterange").on("show.daterangepicker", function (ev, picker) {
      ...
      if (
        $(".AutoUpdateButton").bootstrapSwitch("state") &&
        AUTO_UPDATE_ACTIVE
      ) {
        auto_update_click();
      }
    ...
    $("#daterange").on("hide.daterangepicker", function (ev, picker) {
      if (
        !$(".AutoUpdateButton").bootstrapSwitch("state") &&
        PREVIOUS_AUTO_UPDATE_ACTIVE_STATE
      ) {
        auto_update_click();
      }
    });
  ...
  var event = new CustomEvent("pyscadaDateTimeChange", {
    detail: { picker: $("#daterange").data("daterangepicker") },
    bubbles: false,
    cancelable: true,
    composed: false,
  });
...
function setCrosshairs(flotPlot, id) {
  ...
  $.each(PyScadaPlots, function (plot_id) {
    if (
      flotPlot.crosshair_x !== -1 &&
      flotPlot.crosshair_x !== 0 &&
      !CROSSHAIR_LOCKED
    ) {
      if (
        typeof PyScadaPlots[plot_id].getFlotObject() !== "undefined" &&
        typeof PyScadaPlots[plot_id].getFlotObject().getOptions !==
          "undefined" &&
        PyScadaPlots[plot_id].getFlotObject().getOptions().xaxes.length ===
          pOpt.xaxes.length
      ) {
        if (
          PyScadaPlots[plot_id].getFlotObject().getOptions().xaxes.length ===
            1 &&
          pOpt.xaxes.length === 1 &&
          PyScadaPlots[plot_id].getFlotObject().getOptions().xaxes[0].key ===
            pOpt.xaxes[0].key
        ) {
          $(".chart-legend-value-" + PyScadaPlots[plot_id].getId()).removeClass(
            "type-numeric"
          );
          setTimeout(PyScadaPlots[plot_id].updateLegend(), 50);
          if (PyScadaPlots[plot_id].getId() == id) {
            PyScadaPlots[plot_id].getFlotObject().getOptions().crosshair.mode =
              "xy";
            ...
            PyScadaPlots[plot_id].getFlotObject().getOptions().crosshair.mode =
              "x";
            var x =
              flotPlot.crosshair_x +
              flotPlot.getPlaceholder().offset().left +
              flotPlot.getPlotOffset().left -
              PyScadaPlots[plot_id].getFlotObject().getPlaceholder().offset()
                .left -
              PyScadaPlots[plot_id].getFlotObject().getPlotOffset().left;
            x = Math.max(
              0,
              Math.min(x, PyScadaPlots[plot_id].getFlotObject().width())
            );
            PyScadaPlots[plot_id]
              .getFlotObject()
              .setCrosshair(
                PyScadaPlots[plot_id].getFlotObject().c2p({ left: x })
              );
          }
        } else {
          PyScadaPlots[plot_id].getFlotObject().getOptions().crosshair.mode =
          ...
          $(".chart-legend-value-" + PyScadaPlots[plot_id].getId()).addClass(
            "type-numeric"
          );
        }
      }
...
function delCrosshairs(flotPlot) {
  $.each(PyScadaPlots, function (plot_id) {
    if (
      typeof PyScadaPlots[plot_id].getFlotObject() !== "undefined" &&
      typeof PyScadaPlots[plot_id].getFlotObject().getOptions !== "undefined" &&
      typeof PyScadaPlots[plot_id].getFlotObject().setCrosshair !== "undefined"
    ) {
...
function unlockCrosshairs(flotPlot) {
  $.each(PyScadaPlots, function (plot_id) {
    if (
      typeof PyScadaPlots[plot_id].getFlotObject() !== "undefined" &&
      typeof PyScadaPlots[plot_id].getFlotObject().unlockCrosshair !==
        "undefined"
    ) {
...
function lockCrosshairs(x, y) {
  $.each(PyScadaPlots, function (plot_id) {
    if (
      typeof PyScadaPlots[plot_id].getFlotObject() !== "undefined" &&
      PyScadaPlots[plot_id].getFlotObject().getData().length
    ) {
      ...
      y_tmp =
        y -
        flotPlot.getPlaceholder().offset().top -
        flotPlot.getPlotOffset().top;
  ...
  for (cc in c) {
    if (typeof c[cc] == "object") {
      ...
      for (v in b) {
        ...
        if (
          document.querySelector("#aggregation-type-all-select-" + widget_id) !=
            null &&
          !document.querySelectorAll(
            "#aggregation-type-all-select-" +
              widget_id +
              ' option[value="' +
              v +
              '"]'
      ...
      c[cc].onchange = function () {
        ...
        if (this.value == "null") {
          document.querySelector(
            "#chart-legend-options-span-" + widget_id + "-" + var_id
          ).innerHTML = "";
        } else {
          document.querySelector(
            "#chart-legend-options-span-" + widget_id + "-" + var_id
          ).innerHTML = "(" + this.selectedOptions[0].text + ")";
        }
      };
      document.querySelector(
        "#aggregation-type-all-select-" + widget_id
      ).onchange = function () {
        widget_id = this.dataset["widgetId"];
        c = document.querySelectorAll(
          '.aggregation-type-option[data-widget-id="' + widget_id + '"]'
        );
        for (cc in c) {
        ...
        if (this.value == "null") {
          document.querySelector(
            "#chart-legend-options-span-" + widget_id
          ).innerHTML = "";
        } else {
          document.querySelector(
            "#chart-legend-options-span-" + widget_id
...
function setAggregatedPeriodList(widget_id, var_id) {
  ...
  document.querySelector(
    "li-aggregation-all-period-select-" + widget_id + "-" + var_id
  );
}
 ...
 */
function sliceDATAusingTimestamps(
  data,
  display_from = DATA_DISPLAY_FROM_TIMESTAMP,
  display_to = DATA_DISPLAY_TO_TIMESTAMP,
  from = DATA_FROM_TIMESTAMP,
  to = DATA_TO_TIMESTAMP
) {
...
function timeline_resize(event, ui) {
  ...
  if (window_left < 0.02) {
    if (window_width + window_left < 0.98) {
      DATA_DISPLAY_TO_TIMESTAMP =
        DATA_FROM_TIMESTAMP + min_full * (window_width + window_left);
...
function timeline_drag(event, ui) {
  ...
  if (window_left < 0.02) {
    ...
    DATA_DISPLAY_FROM_TIMESTAMP = DATA_FROM_TIMESTAMP + min_full * window_left;
    DATA_DISPLAY_TO_TIMESTAMP =
      DATA_DISPLAY_FROM_TIMESTAMP + DATA_DISPLAY_WINDOW;
...
function toggle_daterangepicker() {
  ...
  if (window.location.hash.substr(1) !== "") {
    if (
      $("#" + window.location.hash.substr(1) + ".show_daterangepicker").length
    ) {
...
function check_form(id_form) {
  err = false;
  tabinputs = $.merge(
    $("#" + id_form + " :text:visible"),
    $("#" + id_form + " :input:not(:text):hidden")
  );

  for (i = 0; i < tabinputs.length; i++) {
    //test if there is an empty or non numeric value
    ...
    if (value == "" || value == null) {
      ...
      $(tabinputs[i])
        ...
        .append(
          '<span id="helpBlock-' +
            id +
            '" class="help-block">Please provide a value !</span>'
        );
      err = true;
    } else {
      $(tabinputs[i]).parents(".input-group").find(".help-block").remove();
      check_mm = check_min_max(
        parseFloat(value),
        parseFloat(min),
        parseFloat(max),
        min_type,
      ...
      if (check_mm == -1) {
        ...
        $(tabinputs[i])
          ...
          .append(
            '<span id="helpBlock-' +
              id +
              '" class="help-block">Enter a value ' +
              min_type_char +
              " " +
              min +
              "</span>"
          ...
          if (item_type == "variable_property" && value_class == "STRING") {
            ...
            $(tabinputs[i])
              ...
              .append(
                '<span id="helpBlock-' +
                  id +
                  '" class="help-block">The value must be a number ! Use dot not coma.</span>'
              );
            err = true;
          }
        }
  ...
  for (i = 0; i < tabselects.length; i++) {
    //test if there is an empty value
...
function set_loading_state(key, value) {
  ...
  if ($("#page-load-state").length > 0) {
    $("#page-load-state")[0].setAttribute(
      "value",
      Number.parseFloat(loading_states[key]).toFixed(2)
    );
  }
}
...
function add_notification(message, level, timeout, clearable) {
  ...
  if (clearable) {
    $("#notification_area").append(
      '<div id="notification_Nb' +
        NOTIFICATION_COUNT +
        '" class="notification alert alert-' +
        level +
        ' alert-dismissable" style="position: fixed; top: ' +
        top +
        "px; right: " +
        right +
        'px; z-index: 2000"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>' +
        message_pre +
        "</strong>" +
  ...
  if (timeout) {
    setTimeout(
      '$("#notification_Nb' + NOTIFICATION_COUNT + '").alert("close");',
      timeout
    );
  }
...
function raise_data_out_of_date_error() {
  if (!DATA_OUT_OF_DATE) {
    ...
    DATA_OUT_OF_DATE_ALERT_ID = add_notification(
      "displayed data is out of date!",
      4,
      false,
      false
    );
...
function refresh_logo(key, type) {
  ...
  if (type == "variable") {
    ...
    type_short = "prop";
    if (key in VARIABLE_PROPERTIES_LAST_MODIFIED) {
      last_time = VARIABLE_PROPERTIES_LAST_MODIFIED[key];
    }
  }
  type = type.replace("_", "").replace("-", "");
  document
    .querySelectorAll(
      ".control-item.type-numeric." +
        type_short +
        "-" +
        key +
        " img, button.write-task-btn." +
        type_short +
        "-" +
        key +
        " img, button.write-task-set." +
        type_short +
        "-" +
        key +
        " img"
  ...
  if (
    ...
    (get_config_from_hidden_config("variable", "id", key, "readable") ===
      "True" ||
      type !== "variable")
  ) {
    document
      .querySelectorAll(
        ".control-item.type-numeric." +
          ...
          "-" +
          key +
          ", button.write-task-btn." +
          type_short +
          "-" +
          key +
          ", button.write-task-set." +
          type_short +
          "-" +
          key
      )
...
function daterange_cb(start, end) {
  $("#daterange span").html(
    start.format(daterange_format) + " - " + end.format(daterange_format)
  );
}
    ...
    if (isNaN(value)) {
      if (item_type == "variable_property") {
        PYSCADA_XHR = $.ajax({
          ...
          error: function (data) {
            add_notification(
              "Operation not permitted (dropdown " + key + ")",
              3
            );
          },
        });
...
$("input.write-task-btn").click(function () {
  ...
  $("#" + id).removeClass("update-able");
  $(
    ".variable-config[data-refresh-requested-timestamp][data-key=" +
      key +
      "][data-type=" +
      item_type +
      "]"
  ).attr("data-refresh-requested-timestamp", SERVER_TIME);
  $(
    ".variable-config2[data-refresh-requested-timestamp][data-id=" + key + "]"
  ).attr("data-refresh-requested-timestamp", SERVER_TIME);
...
function init_pyscada_content() {
  ...
  fetch(
    "/getHiddenConfig2/" +
      document.querySelector("body").dataset["viewTitle"] +
      "/",
    {
      method: "POST",
      headers: {
    ...
    .then((text) => {
      document.querySelector(
        "body #wrap #content .hidden.globalConfig2"
      ).innerHTML = text;
    ...
    .then(() => {
      ...
      $(".side-menu.right")
        ...
        .mouseleave(function () {
          ...
          $(this)
            .stop()
            .animate({ right: -(ow - 11) }, 500);
        });
      // bottom
      $(".side-menu.bottom").css(
        "margin-left",
        -$(".side-menu.bottom").outerWidth(true) / 2
      );
      $(".side-menu.bottom")
        .stop()
        .animate(
          { bottom: -($(".side-menu.bottom").outerHeight(true) - 31) },
          500
        );
      $(".side-menu.bottom")
        .mouseenter(function () {
        ...
        .mouseleave(function () {
          ...
          $(this)
            .stop()
            .animate({ bottom: -(outerHeight - 31) }, 500);
        });
      ...
      window.onunload = function () {
        for (t in PYSCADA_TIMEOUTS) {
          clearTimeout(PYSCADA_TIMEOUTS[t]);
          console.log("PyScada HMI : clearing timeout", t);
        }
        if (PYSCADA_XHR != null && PYSCADA_XHR.readyState != 4) {
          PYSCADA_XHR.abort();
          for (c in FETCH_CONTROLLERS) {
            FETCH_CONTROLLERS[c].abort();
          }
      ...
      $(".AutoUpdateButton").bootstrapSwitch({
        onInit: function (event) {
          $(".bootstrap-switch-id-AutoUpdateButton").tooltip({
      ...
      $(".dropdown input, .dropdown label, .dropdown button").click(function (
        e
      ) {
        e.stopPropagation();
      });
      ...
      $.each($(".chart-container"), function (key, val) {
        // get identifier of the chart
        id = val.id.substring(16);
        if ($(val).data("xaxis").id == "False") {
          xaxisVarId = null;
        } else {
          xaxisVarId = $(val).data("xaxis").id;
        }
        if ($(val).data("xaxis").linlog == "True") {
          xaxisLinLog = true;
        } else {
          xaxisLinLog = false;
        }
      ...
      document.querySelectorAll(".gauge-container").forEach((e) => {
        ...
        thresholdValues = JSON.parse(
          JSON.parse(e.dataset["params"]).threshold_values
        );
        ...
        for (var v in thresholdValues) {
          ...
          threshold_values.push({
            value: Number(v),
            color: thresholdValues[v],
          });
        }
        // fix threshold values: items must be minimum 3, has the gauge threshold default
        while (
          threshold_values.length < 3 &&
          Object.keys(thresholdValues).length
        ) {
          threshold_values.push({
            value: Number(Object.keys(thresholdValues)[0]),
            color: thresholdValues[Object.keys(thresholdValues)[0]],
          });
        }
        if (threshold_values === "") {
          threshold_values = [];
      ...
      $.each($(".variable-config"), function (key, val) {
        ...
        if (
          VARIABLE_PROPERTY_KEYS.indexOf(key) == -1 &&
          item_type === "variable_property"
        ) {
          VARIABLE_PROPERTY_KEYS.push(key);
        } else if (
          VARIABLE_KEYS.indexOf(key) == -1 &&
          item_type === "variable"
        ) {
          VARIABLE_KEYS.push(key);
        }
        if (
          typeof STATUS_VARIABLE_KEYS[key] == "undefined" &&
          init_type == 0 &&
          item_type === "variable"
        ) {
        ...
        if (
          typeof CHART_VARIABLE_KEYS[key] == "undefined" &&
          init_type == 1 &&
          item_type === "variable"
        ) {
        ...
        if (
          typeof VARIABLE_PROPERTIES[key] == "undefined" &&
          item_type === "variable_property"
        ) {
      ...
      document.querySelectorAll(".transformdata-config2").forEach((e) => {
        if (e.dataset["needHistoricalData"] == "True") {
          displayvalueoption_id = get_config_from_hidden_config(
            "displayvalueoption",
            "transform-data",
            e.dataset["id"],
            "id"
          );
          variable_id = get_config_from_hidden_config(
            "controlitem",
            "display-value-options",
            displayvalueoption_id,
            "variable"
          );
      ...
      $("#timeline").resizable({
        ...
        start: function (event, ui) {
          progressbar_resize_active = true;
        },
      ...
      $(".ReadAllTask").click(function (e) {
        PYSCADA_XHR = $.ajax({
          ...
          success: function (data) {
            document
              .querySelectorAll(".hidden.variable-config2")
              .forEach(function (e) {
            ...
            document
              .querySelectorAll(".hidden.variableproperty-config2")
              .forEach(function (e) {
                e.setAttribute("data-refresh-requested-timestamp", SERVER_TIME);
              });
            document
      ...
      document.querySelectorAll(".refresh-rate-input").forEach((item) => {
        item.oninput = function () {
          document.querySelectorAll(".refresh-rate-output").forEach((item) => {
      ...
      document.querySelectorAll(".refresh-rate-output").forEach((item) => {
        item.innerHTML = document.querySelector(".refresh-rate-input").value;
      });
      document.querySelectorAll(".refresh-rate-li").forEach((item) => {
        item.classList.remove("hidden");
      });
      document.querySelectorAll(".refresh-rate-divider").forEach((item) => {
        item.classList.remove("hidden");
      });
